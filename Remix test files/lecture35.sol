...
2. external (필수 외부에서 보낼때)
3. payable (이더 보낼때)

왜 쓰는가?
1. 스마트 컨트랙이 이더를 받을 수 있게 함
...
fallback은 receive와 fallback으로 두가지 형태로 나뉘게 되었음
...
기본형: 불려진 함수가 특정 스마트 컨트랙이 없을때 fallback 함수 작동
fallback() external{

    }

payable 적용시 : 이더를 받고 나서도 fallback 함수가 작동
